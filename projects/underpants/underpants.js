...
_.indexOf = function(array, value){
 ...
 let index = null;
   ...
   // if index is never changed inside the loop return negative.
 if (index === null){
     return -1;
...
_.unique = function(array){
 let newVar=[];
    //make a for loop to run through array
    for (let i = 0; i< array.length; i++){
        // check if the index is equal to -1 using the _.indexOf function
        if (_.indexOf(newVar, array[i]) === -1){
            // push all the elements in the array in newVar
            newVar.push(array[i]);
        }
    }
    // return newVar
...
_.filter = function(collection, funcky){
 let newArr = [];
 _.each(collection,(element, index, collection) => {
                 if(funcky(element,index, collection)){
                      newArr.push(element);
             }
...
_.reject = function(collection, func){
 let newArr = [];
 _.filter(collection,(element, index, collection) => {
...
_.partition = function(array, funck){
let newArray=[];
let otherArr = [];
let collection = [];
...
_.extend = function(object1,...restobject2){
  // if object2 is an array of objects make a for loop 
  // andd make a for in loop to loop over the objects inside the array
  // use bracket notation to add each property to the next object
